...
@app.post("/classificar_base_doutrinaria")
def classificar_base(req: TextoSimples):
    return {"doutrina": "majoritária (simulado)"}
...
def detectar_keywords(req: TextoSimples):
    ...
    return {"palavras_chave": palavras[:5]}

@app.post("/gerar_estrutura_revisao")
def gerar_estrutura(req: TextoSimples):
    return {"estrutura": ["Tópicos", "Flashcards", "Mapa mental"]}

@app.post("/flashcard_builder")
def flashcard_builder(req: TextoSimples):
    return {"flashcards": [{"pergunta": "O que é?", "resposta": "Simulado"}]}

@app.post("/quiz_generator")
def quiz_generator(req: TextoSimples):
    return {"quiz": [{"questao": "O que diz o art. 5º?", "gabarito": "É direito fundamental"}]}

@app.post("/summary_engine")
def summary_engine(req: TextoSimples):
    return {"resumo": req.conteudo[:100] + "..."}

@app.post("/reflexion_chain")
def reflexion_chain(req: TextoSimples):
    return {"ajuste": "Detectada falha em estrutura — correção aplicada"}

@app.post("/self_diagnostic_module")
def self_diag(req: TextoSimples):
    return {"verificacao": "Bloco de jurisprudência ausente"}
